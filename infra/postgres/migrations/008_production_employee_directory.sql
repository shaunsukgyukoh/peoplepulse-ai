...
CREATE TABLE IF NOT EXISTS core.employee_directory (
    ...
    self_report_status TEXT CHECK (
        self_report_status IS NULL OR self_report_status IN (
            'good', 'okay', 'needs_support', 'prefer_not_to_say'
        )
    ),
    self_report_updated_at TIMESTAMPTZ,
...
'Production HR directory. Stores manager-visible identity and voluntary self-report status. Slack-derived NLP scores are not stored here.';
...
COMMENT ON COLUMN core.employee_directory.self_report_status IS
'Voluntary self-report status. Do not populate from Slack-derived NLP or other inferred mental-health signals.';
